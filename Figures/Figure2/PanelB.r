...
rect_data <- read.table("Rect.box.base.V2.txt",head=T)
...
# SNPs
rect_snp_hash <- read.table("Rect.snp.txt",head=T)
# ypos    xpos
...
L <- read.table("Rect.labels_cov.txt",head=T)
labels <- c("",L$Label)

ggplot() +
  geom_rect(data = rect_data, aes(xmin = xstart, xmax = xend, ymin = ystart, ymax = yend), fill = rect_data$fill, color = "white", alpha = 0.7) +
  geom_segment(data= rect_snp_hash,aes(x=xpos,xend=xpos,y=ypos-0.6,yend=ypos+0.6,color=type),linetype="solid") + scale_color_manual(values=colorsQ[1:2]) +
  ...
  scale_y_continuous(breaks = seq(0, 5) * (1 + gap) + gap / 2 -1.2, labels = labels) +
  theme_minimal() +
