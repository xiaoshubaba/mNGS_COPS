...
# 创建长方形的数据框
rect_data <- read.table("RNA_k75_9508.Rect.box.base.V2.txt",head=T)
#RNA_k75_9508.Rect.box.base.V2.txt
...
ggplot() +
  ...
  scale_y_continuous(breaks = seq(0, 5) * (1 + gap) + gap / 2 -1.2, labels = labels) + # 将纵坐标设置为自定义标签
  theme_minimal() +
  theme(axis.text.y = element_text(size = 10), # 调整纵坐标标签大小
