...
#top
#left
Main <- read.table("BNY.feature.txt",head=T,sep="\t")
Main$Specy = factor(Main$Specy)
Main$log_meanDepth = log(Main$meanDepth)
Main$S1_log_meanDepth = log(Main$S1_meanDep)
Main$ifCops = paste(Main$frequency>=3,Main$S1_fre<10,Main$Specy)
#right
colorsQ = c("grey50","grey50",C[1])
...
f1_right = ggscatter(Main_m6_m10,"gc","log_meanDepth",color="ifCops",alpha=I(0.9),legend="none",palette=C[1],size="length",title=paste0("contigs:",dim(Main_m6_m10)[1]," Signal Ratio:",CalSR(Main_m6_m10)))
# bottom
